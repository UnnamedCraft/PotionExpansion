...
public class ResearchSetup {
    public static void setup(@Nonnull PotionExpansion plugin) {
        new Research(new NamespacedKey(plugin, "potions_sight"), 31001, "矿物透视药水", 28)
        ...
        new Research(new NamespacedKey(plugin, "alchemic_station"), 31003, "高级酿造台", 15)
                .addItems(PotionItems.ALCHEMIC_STATION)
                .register();

        plugin.getLogger().info("研究载入完成!");
    }
}
